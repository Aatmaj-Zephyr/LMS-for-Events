...
def displayEvents():
    ...
    ]]  # lot to feed in put_table function contains a list of list.
    for i in Eventlist:
    ...
    with pout.use_scope('scope1'):
        ...
        pout.put_button("Back", onclick=Continue).style(
            'font-family: "Lucida Console", "Courier New", monospace;font-weight:bold;'
        )
...
def addEventWeb():
    ...
    :return: none
    '''
    data = pimp.input_group(
        "Add Event", [
            pimp.input('Enter Event Name',
                       placeholder='Pls Enter Event Name',
                       name='name',
                       required=True),
            pimp.input('Enter Date of Event',
                       ...
                       type=pimp.DATE,
                       required=True),
            pimp.input('Enter info about the event',
                       name='info',
                       required=True,
                       placeholder='Pls Enter Event Details')
        ],
...
def deleteEventWeb():
    ...
    data = pimp.input_group(
        "Delete Event", [
            pimp.input('Enter name of the event you want to delete',
                       name='name')
        ],
    ...
    with pout.use_scope('scopeV', clear=True):
        pout.put_row([
            pout.put_button("Add Event",
                            onclick=addEventWeb),  # a group of buttons
            pout.put_button("Delete Event",
                            onclick=deleteEventWeb),  # a group of buttons
...
def Register():
    ...
    data3 = pimp.input_group("Register", [
        ...
        pimp.input('Enter Password',
                   ...
                   type=pimp.PASSWORD,
                   required=True),
        pimp.input('Enter Verif Code', name='code', required=True),
    ],
                             validate=is_valid,
                             cancelable=True)
    if ((data3) == None):
...
def displayEvents2():
    ...
    with pout.use_scope('scope1'):
        pout.put_table(displaylist)
        pout.put_button("Back", onclick=restart).style(
            'font-family: "Lucida Console", "Courier New", monospace;font-weight:bold;'
        )
...
with pout.use_scope('scopeLogReg'):
    ...
    pout.put_row(
        [
            pout.put_button("Login", onclick=Login_func,
                            color='success'),  # a group of buttons
            pout.put_button("Register", onclick=Register, color='warning'),
            pout.put_button("Display Events", onclick=displayEvents2)
        ],
        size='30%'
    ).style(
        'font-family: "Lucida Console", "Courier New", monospace;font-weight:bold;font-size:80px;margin-color:solid green'
    )  # a group of buttons
